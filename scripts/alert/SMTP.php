...
class SMTP
{
    ...
    /* Private Variables */
    private $sock;
    private $custom_from_name; /****customize***/
    private $custom_from_address; /****customize***/
    ...
    /****customize***/
    public function set_from($name, $address){
        $this->custom_from_name = $name;
        $this->custom_from_address = $address;
    }
	/****customize***/

    /* Main Function */
    public function sendmail($to, $from, $subject = "", $body = "", $mailtype, $cc = "", $bcc = "", $additional_headers = "", $blacklist = false)
    {
        if($blacklist){
            $to = $this->blacklist($to);
            $cc = $this->blacklist($cc);
            $bcc = $this->blacklist($bcc);
        }
    ...
    private function blacklist($to){
        $blacklist = APF::get_instance()->get_config("blacklist");
        if($blacklist && $blacklist){
            $to = array_filter(array_diff(explode(',', $to),explode(',', $blacklist)));
        }
        return join(',', $to);
    }
}
